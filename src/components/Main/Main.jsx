...
const Main = () => {
  ...
  const handleDelete = (index, cityIndex) => {
    ...
    setHighlightedCities((prev) =>
      prev.filter((highlightedIndex) => highlightedIndex !== cityIndex),
    );
  ...
  const handleDescriptionChange = (index, value) => {
    setWeatherData((prevData) =>
      prevData.map((data, i) =>
        i === index ? { ...data, description: value } : data,
      ),
    );
  };
  ...
  const handleSearch = () => {
    const cityIndex = weatherData.findIndex(
      ...
      <div className="main">
        <div className="aside">
          <button
            ...
            disabled={currentCityIndex >= cities.length}
          >
            Get Weather
          </button>
          <p>City</p>
            ...
            <tbody>
              {cities.map((city, index) => (
                <tr
                  key={city}
                  style={{
                    border: highlightedCities.includes(index)
                      ? "2px solid #00F700"
                      : "1px solid gray",
                  }}
                >
              ...
              {loading ? (
                ...
                weatherData.map((data, index) => (
                  <tr
                    ...
                    style={{
                      backgroundColor:
                        highlightedRow === index ? "yellow" : "transparent",
                    }}
                  >
                    ...
                    <td style={{ border: "1px solid black" }}>
                      {data.temp_in_celsius} °C
                    </td>
                    <td style={{ border: "1px solid black" }}>
                      {data.pressure_in_hPa} hPa
                    </td>
                    <td style={{ border: "1px solid black" }}>
                      {data.humidity_in_percent} %
                    </td>
                    <td style={{ border: "1px solid black" }}>
                      {calculateDataAge(data.date_and_time)} hrs
                    </td>
                    <td style={{ border: "1px solid black" }}>
                      <button
                        onClick={() =>
                          handleDelete(index, cities.indexOf(data.city))
                        }
                      >
                        Delete
                      </button>
